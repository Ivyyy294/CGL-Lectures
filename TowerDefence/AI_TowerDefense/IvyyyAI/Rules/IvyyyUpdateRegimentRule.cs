...
namespace AI_Strategy
{
    public class IvyyyUpdateRegimentRule : IvyyyRule
    {
        ...
        {
            m_axis.Add(new IvyyyRuleAxis("ActiveRegimentSettings", new IvyyyResponseCurve(IvyyyResponseCurve.CurveType.Linear, -1f, 1f, 1f, 0f)));
            m_axis.Add(new IvyyyRuleAxis("DeployTowers", new IvyyyResponseCurve(IvyyyResponseCurve.CurveType.Linear, -1f, 1f, 1f, 0f)));
            m_weight = 10f;
        }

        public override void Action(object target)
        {
            DebugLogger.Log("#Player" + m_worldState.Player.Name + " Update Regiment!");
